   ...
   for (let i = 0; i < 7; i++) {
        ...
        for (let j = 0; j < 7; j++) {
          let countAll=j+count7
            coin= {id:countAll, value: 0, x: i, y: j}
           y.push(coin);
        }
        boardMatrix.push(y)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
  dropCoin = (i) => {                   //GLAVNA funkcija    ubacivanje coin-a u verticalu i ubacivanje verticale u boardV
    let { boardMatrix, coin, change, cId, message } = this.state;
    // boardV = boardV.slice();
...
console.log('-'+i+'-')
    ...
    // boardMatrix = boardMatrix.slice();
    // let boardMatrix = boardMatrix.length;   // uzimanje duzine verticale

    // boardMatrix.push(coin);       // ubacivanje coina u verticalu
    // boardMatrix.splice(i, 1, boardMatrix)  // ubacivanje tj zamena stare verticale, kliknutom verticalom u boardV
  ...
  render() {
    let board = null;
    board = this.state.boardMatrix.map((board, i) => {
      return <Board key={i} board={board} clicked={() => this.dropCoin(i)} boardMatrix={this.state.boardMatrix}/>
    })
